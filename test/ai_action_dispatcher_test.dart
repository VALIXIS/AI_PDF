...
void main() {
  ...
  setUpAll(() async {
    hiveTempDir = await Directory.systemTemp.createTemp('ai_dispatcher_hive_test_');
  ...
  group('AiActionDispatcher Unit Tests', () {
    ...
    test('executeAction executes rotate command on real PDF', () async {
      ...
      pdf.addPage(pw.Page(build: (pw.Context context) => pw.Text('Sample PDF Text')));
    ...
    test('executeAction executes watermark command on real PDF', () async {
      ...
      final pdf = pw.Document();
      pdf.addPage(pw.Page(build: (pw.Context context) => pw.Text('Sample Text')));
      await samplePdf.writeAsBytes(await pdf.save());
